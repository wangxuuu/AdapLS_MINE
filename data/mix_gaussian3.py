...
class MixedGaussian():
    ...
    def __init__(self, sample_size=400, mean1=0, mean2=0, rho1=0.9, rho2=-0.9, rho3=0.5, mix=0.3, mix2=0.3, mix3=0.4):
        ...
        self.covMat3 = np.array([[1, rho3], [rho3, 1]])
        self.sample_size = sample_size
        self.mix = mix
        self.mix2 = mix2
        self.mix3 = mix3
    ...
    def data(self):
        ...
        N2 = int(self.mix2*self.sample_size)
        ...
        temp2 = np.random.multivariate_normal(mean=-self.mu,
                                              cov=self.covMat2,
                                              size=N2)
        temp3 = np.random.multivariate_normal(mean=-self.mu,
                                              cov=self.covMat3,
                                              size=N3)
        X = np.append(temp1, temp2, axis=0)
        X = np.append(X, temp3, axis=0)
        np.random.shuffle(X)
        return X
    ...
    def ground_truth(self):
        ...
        mix, mix2, mix3, covMat1, covMat2, covMat3, mu = self.mix, self.mix2, self.mix3, self.covMat1, self.covMat2, self.covMat3, self.mu
    ...
    def I(self, x,y):
        ...
        mix2, mix3, covMat3, mu3 = self.mix2, self.mix3, self.covMat3, self.mu
        def fxy(x,y):
            X = np.array([x, y])
            ...
            temp3 = np.matmul(np.matmul(X-mu3 , np.linalg.inv(covMat3)), (X-mu3).transpose() )
            return mix*np.exp(-.5*temp1) / (2*np.pi * np.sqrt(np.linalg.det(covMat1))) \
                    + mix2*np.exp(-.5*temp2) / (2*np.pi * np.sqrt(np.linalg.det(covMat2))) \
                    + mix3*np.exp(-.5*temp3) / (2*np.pi * np.sqrt(np.linalg.det(covMat3)))

        def fx(x):
            return mix*np.exp(-(x-mu1[0])**2/(2*covMat1[0,0])) / np.sqrt(2*np.pi*covMat1[0,0]) \
                    + mix2*np.exp(-(x-mu2[0])**2/(2*covMat2[0,0])) / np.sqrt(2*np.pi*covMat2[0,0])\
                    + mix3*np.exp(-(x-mu3[0])**2/(2*covMat3[0,0])) / np.sqrt(2*np.pi*covMat3[0,0])

        def fy(y):
            return mix*np.exp(-(y-mu1[1])**2/(2*covMat1[1,1])) / np.sqrt(2*np.pi*covMat1[1,1]) \
                    + mix2*np.exp(-(y-mu2[1])**2/(2*covMat2[1,1])) / np.sqrt(2*np.pi*covMat2[1,1])\
                    + mix3*np.exp(-(y-mu3[1])**2/(2*covMat3[1,1])) / np.sqrt(2*np.pi*covMat3[1,1])
