# estimate the mix varibales case
import torch
...
def _div2(net, data, ref, disc_data):
    # Calculate the divergence estimate using a neural network
    outputs = net(data)
    outputs_ref = net(ref)
    idx_uniform = bernoulli.rvs(0.5, size=ref.shape[0])
    idx_uni0 = np.where(idx_uniform==0)[0]
    idx_uni1 = np.where(idx_uniform==1)[0]
    idx_0 = np.where(disc_data.cpu()==0)[0]
    idx_1 = np.where(disc_data.cpu()==1)[0]

    g = torch.cat((outputs[idx_0, 0], outputs[idx_1, 1]), dim=0)
    g_ref = torch.cat((outputs_ref[idx_uni0, 0], outputs_ref[idx_uni1, 1]), dim=0)
    mean_f = g.mean()
    log_mean_ef_ref = torch.logsumexp(g_ref, 0) - np.log(ref.shape[0])
    return mean_f - log_mean_ef_ref
...
class MINEE():
    ...
    def __init__(self, X_con, X_disc, Y, batch_size=32, ref_batch_factor=1, lr=1e-3, hidden_size=100):
        self.lr = lr
        self.batch_size = batch_size
        self.ref_batch_factor = ref_batch_factor
        self.X_con = X_con
        self.X_disc = X_disc
        self.X = torch.cat((X_disc, X_con), dim=1) # Combine the two components of X
        self.Y = Y
        self.XY_con = torch.cat((self.X_con, self.Y), dim=1)
        self.XY = torch.cat((self.X, self.Y), dim=1) 

        self.X_ref = _uniform_sample(self.X, batch_size=int(
            self.ref_batch_factor * self.X.shape[0]))
        self.Y_ref = _uniform_sample(Y, batch_size=int(
            self.ref_batch_factor * Y.shape[0]))

        self.XY_net = MINEE.Net2(
            input_size=X_con.shape[1]+Y.shape[1], hidden_size=100)
        self.X_net = MINEE.Net2(input_size=X_con.shape[1], hidden_size=100)
    ...
    def step(self, iter=1):
        ...
        for i in range(iter):
            ...
            batch_XY_disc = batch_XY[:,0] 
            batch_XY_con = batch_XY[:,1:]

            batch_X = _resample(self.X, batch_size=self.batch_size)
            batch_X_disc = batch_X[:,0] 
            batch_X_con = batch_X[:,1:]

            batch_Y = _resample(self.Y, batch_size=self.batch_size)

            batch_X_ref = _uniform_sample(self.X_con, batch_size=int(
            ...
            batch_loss_XY = -_div2(self.XY_net, batch_XY_con, batch_XY_ref, batch_XY_disc)
            batch_loss_XY.backward()
            self.XY_optimizer.step()

            batch_loss_X = -_div2(self.X_net, batch_X_con, batch_X_ref, batch_X_disc)
    ...
    def forward(self, X=None, Y=None):
        ...
        """
        X_ref = _uniform_sample(self.X_con, batch_size=int(
            self.ref_batch_factor * self.batch_size))
        Y_ref = _uniform_sample(self.Y, batch_size=int(
            self.ref_batch_factor * self.batch_size))
        XY_ref = torch.cat((X_ref, Y_ref), dim=1)

        dXY = _div2(self.XY_net, self.XY_con, XY_ref, self.X_disc).cpu().item()
        dX = _div2(self.X_net, self.X_con, X_ref, self.X_disc).cpu().item()
        dY = _div(self.Y_net, self.Y, Y_ref).cpu().item()
        return dXY, dX, dY
