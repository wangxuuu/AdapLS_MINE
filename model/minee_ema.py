...
class MINEE():
    ...
    def __init__(self, X, Y, batch_size=32, ref_batch_factor=1, lr=1e-3, hidden_size=100,ma_rate=0.1,ma_ef=1):
        self.lr = lr
    ...
    def step(self, iter=1):
        ...
        for i in range(iter):
            ...
            # train X_net
            mean_fX = self.X_net(batch_X).mean()
            mean_efXY_ref = torch.exp(self.X_net(batch_X_ref)).mean()
            self.ma_ef1 = (1-self.ma_rate)*self.ma_ef1 + \
                self.ma_rate*mean_efXY_ref
            batch_loss_X = - mean_fX + \
                (1 / self.ma_ef1).detach() * mean_efXY_ref
            batch_loss_X.backward()
            self.X_optimizer.step()
            # train Y_net
            mean_fY = self.Y_net(batch_Y).mean()
            mean_efXY_ref = torch.exp(self.Y_net(batch_Y_ref)).mean()
            self.ma_ef2 = (1-self.ma_rate)*self.ma_ef2 + \
                self.ma_rate*mean_efXY_ref
            batch_loss_Y = - mean_fY + \
                (1 / self.ma_ef2).detach() * mean_efXY_ref
            ...
            self.ma_ef3 = (1-self.ma_rate)*self.ma_ef3 + \
                self.ma_rate*mean_efXY_ref
            batch_loss_XY = - mean_fXY + \
                (1 / self.ma_ef3).detach() * mean_efXY_ref
